...
function out = plotActivationMap(cPath,Animal,Rec,inds,plttitle,pltlegend,clims,fsize, meansubtract, suppress)
...
for i = 1:length(inds) %will iterate thru each trial set and calculate a PSTH for those
    ...
    for j = 1:length(frames)-1 %iterate thru trial periods
        if meansubtract
            movie = alignedVc(:,frames(j):frames(j+1)) - nanmean(alignedVc(:,1:frames(2)),2);
        else
