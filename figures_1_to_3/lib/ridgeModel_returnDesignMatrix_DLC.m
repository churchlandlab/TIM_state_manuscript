function [regLabels,regIdx,fullR,regZeroFrames,zeromeanVc,U,usedTrials] = ridgeModel_returnDesignMatrix_DLC(cPath,Animal,Rec,glmPath,desiredstate,dType,randomizeStates,orthogvideotoinstructed)
...
%% Load the DLC data
[dlc_unaligned_lat, event_frames_lat] = loadDLCLabels('X:\chaoqun_DLC_labels',Animal, Rec, 'Lateral'); %lateral cam
[dlc_unaligned_bot, event_frames_bot] = loadDLCLabels('X:\chaoqun_DLC_labels',Animal, Rec, 'Bottom'); %bottom cam

alignmentEvent = 'stimGrab';
bhvsRate = 30; %framerate of behavior cams
preVidFrames = sRate * preStimDur * (bhvsRate / sRate);
postVidFrames = sRate * postStimDur * (bhvsRate / sRate);

dlc_aligned_lat = alignDLCToBehavior(dlc_unaligned_lat, event_frames_lat.(alignmentEvent), preVidFrames, postVidFrames);
dlc_aligned_lat = dlc_aligned_lat(bTrials,:,:);
dlc_aligned_bot = alignDLCToBehavior(dlc_unaligned_bot, event_frames_bot.(alignmentEvent), preVidFrames, postVidFrames);
dlc_aligned_bot = dlc_aligned_bot(bTrials,:,:);

dlc_aligned = cat(2, dlc_aligned_lat,dlc_aligned_bot);

%DLC_LABELS_TO_KEEP = [1:42,49:52]; %exclude some labels (ie. handles)
%DLC_LABELS_TO_KEEP = [1:22,29:42,49:52]; %exclude some labels (ie. handles, hands)

DLC_LABELS_TO_KEEP = [1:52]; %exclude handles only


load('X:\DLC_label_names.mat');
DLC_labels = DLC_labels(DLC_LABELS_TO_KEEP);
dlc_aligned = dlc_aligned(:,DLC_LABELS_TO_KEEP,:);

%DLC_LABELS_TO_ORTHOGONALIZE = [23:28, 43:48]

%compute motion energy
dlc_aligned_me = [];
for i = 1:size(dlc_aligned,1)
    for j = 1:2:size(dlc_aligned,2)
        xy = squeeze(dlc_aligned(i,j:j+1,:));
        xy_motion_energy = diff(xy');
        motion_energy = sqrt(xy_motion_energy(:,1).^2 + xy_motion_energy(:,2).^2);
        motion_energy = [NaN;squeeze(motion_energy)];
        motion_energy = filloutliers(motion_energy,'spline',"percentiles",[1 99.9]); % remove outliers from jumping points
        dlc_aligned_me(i,(j+1)/2,:) = motion_energy;
    end
end
DLC_me_labels = DLC_labels(1:2:end);
DLC_me_labels = arrayfun(@(x) strrep(x,'_x','_me'), DLC_me_labels);

rsampl = []; resampme = [];
for i = 1:size(dlc_aligned_lat,1)
    if all(all(isnan(dlc_aligned(i,:,:))))
        rsampl(i,:,:) = NaN;
        rsampme(i,:,:) = NaN;
    else
        rsampl(i,:,:) = Behavior_vidResamp(squeeze(dlc_aligned(i,:,:))', bhvsRate, sRate);
        rsampme(i,:,:) = Behavior_vidResamp(squeeze(dlc_aligned_me(i,:,:))', bhvsRate, sRate);
    end
end

dlc_aligned = reshape(permute(rsampl,[3,2,1]), size(rsampl,3),[]); %reshape to 2D (remove trials)
dlc_aligned_me = reshape(permute(rsampme,[3,2,1]), size(rsampme,3),[]); %reshape to 2D (remove trials)

%standardize them
dlc_aligned = zscore(dlc_aligned,[],2);
dlc_aligned_me = zscore(dlc_aligned_me,[],2);
clear rsampl rsampme
%% build digital regressors for DLC motion energy
% for i=1:size(dlc_aligned_me,1)
%     traceIn = dlc_aligned_me(i,:)';
%     figure;
%     %plot(traceIn)
%     %plot((traceIn - prctile(traceIn,1))./ nanstd(traceIn))
%     plot(zscore(traceIn))
% end
dlc_threshold = 2;
for i=1:size(dlc_aligned_me,1)
    ff = Widefield_analogToDesign(dlc_aligned_me(i,:)', dlc_threshold, trialCnt, sRate, sRate, motorIdx, gaussShift);
    DLC_digital_regs{i} = cat(1,ff{:});
end
DLC_me_digital_labels = arrayfun(@(x) strcat(x, '_digital'), DLC_me_labels);
...
fullR = {timeR ChoiceR rewardR lGrabR rGrabR  ...
    ...
    nextChoiceR waterR piezoAnalog piezoDigital piezoMoveAnalog piezoMoveDigital fastPupilAnalog fastPupilDigital  ...
    slowPupilR, vidR, moveR};
fullR = [fullR num2cell(dlc_aligned',1) num2cell(dlc_aligned_me',1) DLC_digital_regs];
%fullR = [fullR num2cell(dlc_aligned_me',1) DLC_digital_regs];
...
regLabels = {
    ...
    'prevReward' 'prevChoice' 'nextChoice' 'water' 'piezoAnalog' 'piezoDigital' 'piezoMoveAnalog' 'piezoMoveDigital' ...
    'fastPupilAnalog' 'fastPupilDigital' 'slowPupil', 'vidR' 'moveR'};
regLabels = cat(2,regLabels, cellstr(DLC_labels), cellstr(DLC_me_labels), cellstr(DLC_me_digital_labels));
%regLabels = cat(2,regLabels, cellstr(DLC_me_labels), cellstr(DLC_me_digital_labels));

%index to reconstruct different response kernels
regIdx = [];
for i = 1:length(regLabels)
    t = ones(1,size(fullR{i},2))*find(ismember(regLabels,regLabels{i}));
    regIdx = [regIdx, t];
end
...
regZeroFrames = NaN(size(regIdx));

%%

% orthogonalize video against spout/handle movement
regs2orthog = {'DLC_Jaw','DLC_Elbow','DLC_Hand','DLC_Tongue','DLC_Mouth','DLC_Shoulder','vidR','moveR'};
regs2orthog = {'vidR','moveR'};
vidIdx = find(ismember(regIdx, find(contains(regLabels,regs2orthog)))); %index for video regressors
...
smallR = [leverInR spoutR spoutOutR]; % new orthog
...
%figure;imshow(temp(1:2000,:))
%figure;imshow(fullR(1:2000,:))
