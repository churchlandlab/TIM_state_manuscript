function [Lateral_allFrames, Bottom_allFrames, aligned_FrameTime, raw_data, FilteredPupil, delete_idx] = FrameProcessing...
    (fPath, Lateral_allFrames, Bottom_allFrames, aligned_FrameTime, raw_data, FilteredPupil)
...
if a~=b || a~=c || b~=c
    message = 'Please make sure the lateral camera file, bottom camera file, and frame alignment file have SAME trial numbers!';
...
for a = 1 : size(Bottom_allFrames.frames,2)
    ...
    % 31~36 denote the locations and likelihood of two water spots (used as tracking references).
    % 22~24 denote the locations and likelihood of the tongue, which is invisiable in most of the frames.
end
...
% "original_OrderIdx" is used for recovering the original indice, the session names, and the number of trials of each session.


idx = find(isnan(aligned_FrameTime.stimOn(1,:)));
idx = [idx, find(isnan(aligned_FrameTime.stimOff(1,:)))];
...
if ~isempty(FilteredPupil)
    FilteredPupil.orgPupil(idx) = [];
    FilteredPupil.fillPupil(idx) = [];
%     FilteredPupil.sPupil(idx) = [];
%     FilteredPupil.fPupil(idx) = [];
end
...
%% 4th, Normalization
global Lateral_allFrames_raw Bottom_allFrames_raw       
% Keep the raw DLC results without normalization for calculating the motion energy & average movetion
Lateral_allFrames_raw = Lateral_allFrames;
Bottom_allFrames_raw = Bottom_allFrames;
...
lateral_path = dir([fPath mousename '_SpatialDisc_*_rawVideos\' 'Lateral\']);
lateral_path = lateral_path(1).folder;
bottom_path = dir([fPath mousename '_SpatialDisc_*_rawVideos\' 'Bottom\']);
bottom_path = bottom_path(1).folder;
